...
use App\Services\TaskRewardWithdrawalService;
...
class TaskRewardWithdrawalController extends Controller
{
    public function submit()
    {
        /** @var TaskRewardWithdrawalInput $input */
        $input = TaskRewardWithdrawalInput::new();
...
//        }
        ...
        $withdrawAmount = 0;
        $commissionQuery = TaskRewardService::getInstance()
            ->getUserTaskRewardQuery([$this->userId()], [2])
        ...
        DB::transaction(function () use ($withdrawAmount, $input) {
            $withdrawal = TaskRewardWithdrawalService::getInstance()->addWithdrawal($this->userId(), $withdrawAmount, $input);

            if ($input->path == 3) { // 提现至余额
                TaskRewardService::getInstance()->finishWithdrawal($this->userId(), $input->scene, $withdrawal->id);
                AccountService::getInstance()
                    ->updateBalance($this->userId(), AccountChangeType::REWARD_WITHDRAWAL, $withdrawAmount);
            } else {
                TaskRewardService::getInstance()->applyWithdrawal($this->userId(), $input->scene, $withdrawal->id);
    ...
    public function recordList()
    {
        /** @var PageInput $input */
        $input = PageInput::new();
        $page = TaskRewardWithdrawalService::getInstance()->getUserRecordList($this->userId(), $input);
