...
use App\Services\Mall\Hotel\HotelShopDepositChangeLogService;
...
class HotelShopDepositController extends Controller
{
    public function payParams()
    {
        $shopId = $this->verifyRequiredId('shopId');
        $wxPayOrder = HotelShopService::getInstance()
    ...
    public function depositInfo()
    {
        ...
        $deposit = HotelShopDepositService::getInstance()->getShopDeposit($shopId);
        if (is_null($deposit)) {
            return $this->fail(CodeResponse::NOT_FOUND, '保证金数据不存在');
        }

        $shopInfo = HotelShopService::getInstance()->getShopById($shopId);
    ...
    public function changeLogList()
    {
        ...
        $page = HotelShopDepositChangeLogService::getInstance()->getLogPage($shopId, $input);
