...
class HotelProviderOrderService extends BaseService
{
    public function createOrder(int $userId, int $providerId, string $paymentAmount)
    {
        $order = HotelProviderOrder::new();
    ...
    {
        return HotelProviderOrder::query()->where('order_sn', $orderSn)->exists();
    }

    public function getWxPayOrder(int $userId, int $orderId, string $openid)
    {
        $order = HotelProviderOrder::query()->where('user_id', $userId)->find($orderId);
        ...
        return [
            'out_trade_no' => time(),
            'body' => 'hotel_provider_order_sn:' . $order->order_sn,
    ...
    {
        $orderSn = $data['body'] ? str_replace('hotel_provider_order_sn:', '', $data['body']) : '';
    ...
    {
        return HotelProviderOrder::query()->where('order_sn', $orderSn)->first();
    }

    public function getOrderByUserId($userId, $columns = ['*'])
    {
        return HotelProviderOrder::query()->where('user_id', $userId)->first($columns);
    }

    public function getOrderList(PageInput $input, $columns = ['*'])
    {
        return HotelProviderOrder::query()->orderBy($input->sort, $input->order)->paginate($input->limit, $columns, 'page', $input->page);
    }

    public function getOrderListByProviderIds(array $providerIds, $columns = ['*'])
    {
        return HotelProviderOrder::query()->whereIn('provider_id', $providerIds)->get($columns);
    }
}
