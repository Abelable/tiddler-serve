...
class CateringProviderOrderService extends BaseService
{
    public function createOrder(int $userId, int $providerId, string $paymentAmount)
    {
        $order = CateringProviderOrder::new();
    ...
    {
        return CateringProviderOrder::query()->where('order_sn', $orderSn)->exists();
    }

    public function getWxPayOrder(int $userId, int $orderId, string $openid)
    {
        $order = CateringProviderOrder::query()->where('user_id', $userId)->find($orderId);
        ...
        return [
            'out_trade_no' => time(),
            'body' => 'catering_provider_order_sn:' . $order->order_sn,
    ...
    {
        $orderSn = $data['body'] ? str_replace('catering_provider_order_sn:', '', $data['body']) : '';
    ...
    {
        return CateringProviderOrder::query()->where('order_sn', $orderSn)->first();
    }

    public function getOrderByUserId($userId, $columns = ['*'])
    {
        return CateringProviderOrder::query()->where('user_id', $userId)->first($columns);
    }

    public function getOrderList(PageInput $input, $columns = ['*'])
    {
        return CateringProviderOrder::query()->orderBy($input->sort, $input->order)->paginate($input->limit, $columns, 'page', $input->page);
    }

    public function getOrderListByProviderIds(array $providerIds, $columns = ['*'])
    {
        return CateringProviderOrder::query()->whereIn('provider_id', $providerIds)->get($columns);
    }
}
