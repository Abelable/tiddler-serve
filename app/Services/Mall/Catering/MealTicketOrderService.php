...
class MealTicketOrderService extends BaseService
{
    ...
    public function getUnpaidOrder(int $userId, $orderId, $columns = ['*'])
    {
        return MealTicketOrder::query()
            ->where('user_id', $userId)
            ->where('id', $orderId)
            ->where('status', MealTicketOrderStatus::CREATED)
            ->first($columns);
    }
    ...
    public function cancel($userId, $orderId, $role = 'user')
    {
        ...
        if ($order->status != MealTicketOrderStatus::CREATED) {
            $this->throwBusinessException(CodeResponse::ORDER_INVALID_OPERATION, '订单不能取消');
        }
        switch ($role) {
            case 'system':
                $order->status = MealTicketOrderStatus::AUTO_CANCELED;
                break;
            case 'admin':
                $order->status = MealTicketOrderStatus::ADMIN_CANCELED;
                break;
            case 'user':
                $order->status = MealTicketOrderStatus::CANCELED;
                break;
        }
        if ($order->cas() == 0) {
            $this->throwUpdateFail();
    ...
    public function confirm($orderList, $role = 'user')
    {
        $orderList = $orderList->map(function (MealTicketOrder $order) use ($role) {
            ...
            switch ($role) {
                case 'system':
                    $order->status = MealTicketOrderStatus::AUTO_CONFIRMED;
                    break;
                case 'admin':
                    $order->status = MealTicketOrderStatus::ADMIN_CONFIRMED;
                    break;
                case 'user':
    ...
    public function systemFinish()
    {
        ...
        if (count($orderList) != 0) {
            $orderList->map(function (MealTicketOrder $order) {
                ...
                $order->status = MealTicketOrderStatus::AUTO_FINISHED;
                if ($order->cas() == 0) {
                    $this->throwUpdateFail();
                }
            });
    ...
    public function finish($userId, $orderId)
    {
        ...
        $order->status = MealTicketOrderStatus::FINISHED;
        if ($order->cas() == 0) {
            $this->throwUpdateFail();
        }
        return $order;
