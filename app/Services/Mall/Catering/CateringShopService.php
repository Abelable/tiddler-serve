...
class CateringShopService extends BaseService
{
    ...
    {
        return CateringShop::query()->where('user_id', $userId)->where('shop_id', $shopId)->first($columns);
    }
    ...
    public function wxPaySuccess(array $data)
    {
        ...
        $shopDeposit = CateringShopDepositService::getInstance()->getShopDeposit($shopId);
        ...
        if ($shop->status == 0) {
            ...
            // 更新商家状态
            CateringMerchantService::getInstance()->settled($shop->merchant_id);

            // 邀请商家入驻活动
            $userTask = UserTaskService::getInstance()
                ->getByMerchantId(MerchantType::CATERING, $shop->merchant_id, 1);
            if (!is_null($userTask)) {
                $userTask->step = 2;
                $userTask->save();
            }
