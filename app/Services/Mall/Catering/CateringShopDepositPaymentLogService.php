...
use App\Models\CateringShopDepositPaymentLog;
...
class CateringShopDepositPaymentLogService extends BaseService
{
    ...
    {
        $log = CateringShopDepositPaymentLog::new();
        $log->user_id = $userId;
    ...
    {
        return CateringShopDepositPaymentLog::query()->where('shop_id', $shopId)->first($columns);
    }

    public function getLogPage(PageInput $input, $columns = ['*'])
    {
        return CateringShopDepositPaymentLog::query()
    ...
    {
        return CateringShopDepositPaymentLog::query()->whereIn('shop_id', $shopIds)->get($columns);
    }
}
