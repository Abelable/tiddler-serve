...
abstract class TimRestInterface
{
     ...
     */
    abstract public function generate_user_sig($identifier, $expiry_after);
     ...
     * @return array 通过解析REST接口json返回包得到的关联数组，包含新建的群号、成功与否、错误提示等字段
     */
    abstract function group_create_group3($group_type, $group_name, $owner_id, $group_id);
    ...
    /**
     * 获取用户状态
     * @param array $user_ids 用户的id组
     * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
     */
    abstract function query_user_state($user_ids);
}
