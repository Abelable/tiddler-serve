...
class CreateHotelVerifyCodesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('hotel_verify_codes', function (Blueprint $table) {
            $table->id();
            $table->tinyInteger('status')->default(0)->comment('核销状态：0-待核销，1-已核销, 2-已失效');

            $table->unsignedBigInteger('hotel_id')->index()->comment('酒店id');
    ...
    {
        Schema::dropIfExists('hotel_verify_codes');
    }
}
